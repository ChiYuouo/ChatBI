...
app = FastAPI(
    ...
    version="0.1.0",
    description="企业级 ChatBI MVP 的服务化接口，提供同步查询、健康检查与统一异常处理。"
)
...
class QueryRequest(BaseModel):
    ...
    question: str = Field(
        ...,
        min_length=1,
        description="业务人员的自然语言问题"
    )

    use_few_shot: bool = Field(
        default=True,
        description="是否启用 Few-shot 示例"
    )

    use_rules: bool = Field(
        default=True,
        description="是否启用业务规则约束"
    )

    use_guards: bool = Field(
        default=True,
        description="是否启用错误防护"
    )

    use_indicator_knowledge: bool = Field(
        default=True,
...
def query_chatbi(payload: QueryRequest, request: Request) -> QuerySuccessResponse:
    ...
    logger.info(
        "Question handled successfully in %.2f ms",
        duration_ms
    )
